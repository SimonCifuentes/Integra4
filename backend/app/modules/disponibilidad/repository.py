...
def _intervalos_ocupados(db: Session, id_cancha: int, fecha) -> List[Tuple[datetime, datetime]]:
    ...
    # Recupera tanto las reservas como los bloqueos para el día
    sql = """
    ...
    SELECT inicio, fin FROM bloqueos
     WHERE id_cancha=:c
       AND NOT (fin<=:start OR inicio>=:end)
    """
    rows = db.execute(text(sql), {"c": id_cancha, "start": start, "end": end}).all()
    return [(r[0].astimezone(_TZ), r[1].astimezone(_TZ)) for r in rows]
...
def slots_disponibles(db: Session, *, id_cancha: int, fecha, slot_min: int) -> List[Tuple[str, str]]:
    ...
    # Determina si el intervalo [a,b] está libre
    def libre(a: datetime, b: datetime) -> bool:
        # Verifica si el intervalo (a,b) no se solapa con ningún intervalo ocupado
        return all(b <= x or a >= y for (x, y) in ocupados)
