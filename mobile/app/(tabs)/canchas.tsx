...
export default function CanchasScreen() {
  const {
    ...
    refetch,
    isRefetching,
  } = useCanchas({ page: 1, page_size: 20 });
  ...
  const { data: ratingsData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const arr: RatingResumen[] = Array.isArray(data)
        ? data
        : data?.data ?? data?.items ?? [];
      return arr;
    },
  ...
  const canchas = useMemo(() => {
    ...
    return items
      ...
      .sort(
        (a, b) =>
          Number(b.disponible_hoy ?? 0) - Number(a.disponible_hoy ?? 0)
      );
    ...
    <>
      <View style={styles.headerTop}>
        ...
        <Text style={styles.subtle}>
          Explora por deporte, complejo o sector
        </Text>
      </View>
      ...
      <View style={styles.filtersRow}>
        <DropdownChip
          ...
          onPress={() => {
            ...
            if (!fDeporte) {
              ...
              const idx = deportesOpciones.findIndex(
                (d) => normalize(d) === normalize(fDeporte)
              );
              const next =
                idx < 0 || idx + 1 >= deportesOpciones.length
                  ? null
                  : deportesOpciones[idx + 1];
              setFDeporte(next);
            }
          }}
          active={!!fDeporte}
          count={
            fDeporte
              ? canchas.filter(
                  (c) =>
                    normalize(inferDeporte(c)) === normalize(fDeporte)
                ).length
              : undefined
          }
        />
        <DropdownChip
          ...
          onPress={() => {
            ...
            if (!fSector) {
              ...
              const idx = sectoresOpciones.findIndex(
                (s) => normalize(s) === normalize(fSector)
              );
              const next =
                idx < 0 || idx + 1 >= sectoresOpciones.length
                  ? null
                  : sectoresOpciones[idx + 1];
              setFSector(next);
            }
          }}
          active={!!fSector}
          count={
            fSector
              ? canchas.filter(
                  (c) => normalize(c.sector ?? "") === normalize(fSector)
                ).length
              : undefined
          }
        />
        <DropdownChip
          ...
          count={
            fFecha
              ? canchas.filter((c) => c.disponible_hoy).length
              : undefined
          }
        />
      ...
      {isError && !isLoading && (
        <View style={{ paddingHorizontal: 16, paddingVertical: 12 }}>
          <Text style={{ color: "#b91c1c" }}>
            No se pudieron cargar las canchas. Reintenta.
          </Text>
          <TouchableOpacity
            onPress={() => refetch()}
      ...
      <View style={styles.card}>
        ...
        <View style={styles.cardHeader}>
          ...
          <Text style={styles.cardTitle}>
            {item.nombre ?? item.tipo ?? deporteUI}
          </Text>
        </View>
        ...
        <View style={styles.cardBody}>
          ...
          <Text style={styles.text}>
            🏷️ Precio:{" "}
            {typeof item.precio_desde === "number"
              ? `${formatCLP(item.precio_desde)}/h`
              : "—"}
          </Text>
          <Text style={styles.text}>
          ...
          {typeof item.disponible_hoy === "boolean" && (
            <View style={{ marginTop: 6 }}>
              <Badge
                text={
                  item.disponible_hoy ? "Hoy disponible" : "Hoy no hay"
                }
                tone={item.disponible_hoy ? "success" : "muted"}
              />
            </View>
          )}
        </View>
  ...
  if (!isLoading && !isError && canchas.length === 0) {
    return (
      <FlatList
        ...
        ListEmptyComponent={
          <EmptyState text="No encontramos canchas con esos filtros." />
        }
        refreshControl={
          <RefreshControl
...
const styles = StyleSheet.create({
  ...
  ratingCount: { fontWeight: "600", color: "#6b7280" },
