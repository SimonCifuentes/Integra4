...
export default function CanchasPorComplejoScreen() {
  ...
  console.log("canchas-por-complejo params =>", params, "idNum:", idNum);
  ...
  if (isError) {
    return (
      <View style={styles.center}>
        <Ionicons name="alert-circle" size={28} color="#ef4444" />
        <Text style={styles.error}>{(error as any)?.message ?? "No se pudieron cargar las canchas."}</Text>
      ...
      ListHeaderComponent={
        <View style={styles.headerTop}>
          ...
          <View>
            ...
            {!!nombre && <Text style={styles.subtle}>{nombre}</Text>}
          </View>
        </View>
      }
      renderItem={({ item }) => (
...
const styles = StyleSheet.create({
  ...
  retryBtn: { marginTop: 10, backgroundColor: "#111827", paddingHorizontal: 16, paddingVertical: 8, borderRadius: 10 },
  ...
  btnPrimary: { flexDirection: "row", alignItems: "center", justifyContent: "center", backgroundColor: "#0ea5a4", paddingVertical: 10, borderRadius: 10 },
